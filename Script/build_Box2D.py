...
with aceutils.CurrentDir('../Downloads'):
    ...
    if aceutils.isWin():
        ...
        with aceutils.CurrentDir("box2d_bin"):
            aceutils.call(
                r'cmake -D CMAKE_BUILD_TYPE=Release -G "Unix Makefiles" ../' + pathname + r'/Box2D/')
            aceutils.call(r'make')

    if aceutils.isWin():
        aceutils.mkdir(r'../Dev/lib/Debug')
        aceutils.mkdir(r'../Dev/lib/Release')
        aceutils.copytree(pathname + r'/Box2D/Box2D', r'../Dev/include/Box2D/',
                          True, ignoreList=shutil.ignore_patterns(r'*.txt', r'*.cpp', r'*.cmake'))

        aceutils.copy(r'box2d_bin/Debug/Box2D.lib', r'../Dev/lib/x86/Debug/')
        aceutils.copy(r'box2d_bin/Release/Box2D.lib',
                      r'../Dev/lib/x86/Release/')
        aceutils.copy(r'box2d_bin_x64/Debug/Box2D.lib',
                      r'../Dev/lib/x64/Debug/')
        aceutils.copy(r'box2d_bin_x64/Release/Box2D.lib',
                      r'../Dev/lib/x64/Release/')

    elif aceutils.isMac():
        aceutils.copytree(pathname + r'/Box2D/Box2D',
                          r'../Dev/include/Box2D/', True)
        aceutils.copy(
            r'box2d_temp/Box2D/Build/xcode4/bin/Release/libBox2D.a', r'../Dev/lib/')
        # ダミー用にコピー
        aceutils.mkdir(r'box2d_bin/Box2D')
        aceutils.copy(
            r'box2d_temp/Box2D/Build/xcode4/bin/Release/libBox2D.a', r'box2d_bin/Box2D/')
    else:
        aceutils.copytree(pathname + r'/Box2D/Box2D',
                          r'../Dev/include/Box2D/', True)
