...
AmusementCreatorsEngine(以下ACE)により製作されるゲームは主にシーン、レイヤー、オブジェクトの3つの要素で構成されています。
...
一般的なゲームだとシーン間の遷移の際に画面がフェードイン・フェードアウトすることが多いですが、ACEではこの表現をトランジションという機能を使用することで容易に実現できます。

### 時間指定について

ACEは実時間を指定する場合、1/60秒を1とした値で指定します。例えば、1秒は60、0.5秒は30になります。これはディスプレイが60Hzで動作していることに起因します。
