...
public:
	...
	*/
	static bool
	LoadPNGImage(void* data, int32_t size, bool rev, int32_t& imagewidth, int32_t& imageheight, std::vector<uint8_t>& imagedst, Log* log);
...
*/
class Graphics_Imp : public Graphics
{
...
protected:
	...
	{
		return CreateEmptyTexture2D_Imp(width, height, format);
	}
...
protected:
	...
	virtual void
	DrawPolygonInternal(int32_t count, VertexBuffer_Imp* vertexBuffer, IndexBuffer_Imp* indexBuffer, NativeShader_Imp* shaderPtr);
	virtual void DrawPolygonInternal(
		int32_t offset, int32_t count, VertexBuffer_Imp* vertexBuffer, IndexBuffer_Imp* indexBuffer, NativeShader_Imp* shaderPtr);

	virtual void DrawPolygonInstancedInternal(
...
private:
	...
	CreateEmptyTexture2D_Imp_Internal(Graphics* graphics, int32_t width, int32_t height, TextureFormat format, void* data);
...
public:
	...
	static Graphics_Imp* Create(void* handle1, void* handle2, int32_t width, int32_t height, Log* log, File* file, GraphicsOption option);

	static Graphics_Imp* Create(Window* window, GraphicsDeviceType graphicsDevice, Log* log, File* file, GraphicsOption option);

	static Graphics_Imp* Create(void* handle1,
								void* handle2,
								int32_t width,
								int32_t height,
								GraphicsDeviceType graphicsDevice,
	...
	*/
	...
	/*
	virtual CubemapTexture* CreateCubemapTextureFrom6ImageFiles_(const achar* front, const achar* left, const achar* back, const achar*
	right, const achar* top, const achar* bottom) { return nullptr; }
	...
	*/
	std::shared_ptr<VertexBuffer_Imp> CreateVertexBuffer_Imp(int32_t size, int32_t count, bool isDynamic)
	{
	...
	*/
	std::shared_ptr<IndexBuffer_Imp> CreateIndexBuffer_Imp(int maxCount, bool isDynamic, bool is32bit)
	{
	...
	*/
	NativeShader_Imp* CreateShader_Imp_(const char* vertexShaderText,
	...
	*/
	...
	基本的にShaderCacheを経由してシェーダーを生成するため、この関数を直接使う機会は殆ど無い。
	*/
	std::shared_ptr<NativeShader_Imp> CreateShader_Imp(const char* vertexShaderText,
													   const char* vertexShaderFileName,
													   const char* pixelShaderText,
													   const char* pixelShaderFileName,
													   std::vector<VertexLayout>& layout,
													   ...
													   std::vector<Macro>& macro)
	{
		return CreateSharedPtr(
			CreateShader_Imp_(vertexShaderText, vertexShaderFileName, pixelShaderText, pixelShaderFileName, layout, is32Bit, macro));
	}
	...
	*/
	...
	textureがnullの場合は無条件で、デフォルトの描画先を使用する。
	*/
	virtual void SetRenderTarget(RenderTexture2D_Imp* texture1,
