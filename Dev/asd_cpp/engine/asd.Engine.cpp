...
bool Engine::GenerateCore()
{
	...
	public:
		...
		Tool* CreateTool(void* self)
		{
			return new Tool(self, false);
		}
	};
	...
	void Engine::OpenTool()
	{
		...
		tool = accessor.CreateTool(m_core->GetTool());
	}

	void Engine::CloseTool()
	{
		if (tool == nullptr) return;
		SafeDelete(tool);
		m_core->CloseTool();
