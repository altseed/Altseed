...
{
    class CollidingEvent : EngineTest
    {
        class CollidingObject1 : asd.TextureObject2D
        {
            ...
            protected override void OnCollisionEnter(Collision2DInfo collisionInfo)
            {
                Console.WriteLine("Enter: " + collisionInfo.SelfCollider.Tag);
            }
            protected override void OnCollisionStay(Collision2DInfo collisionInfo)
            {
                //printf("Stay:%s\n", asd::ToAString(collisionInfo->GetSelfCollider()->GetTag()).c_str());
            }
            protected override void OnCollisionExit(Collision2DInfo collisionInfo)
            {
