...
//
//----------------------------------------------------------------------------------
#include "ace.RenderState_Imp_DX11.h"
...
//
//----------------------------------------------------------------------------------
RenderState_Imp_DX11::RenderState_Imp_DX11(Graphics_Imp_DX11* graphics)
	: m_graphics(graphics)
{
	...
	for (int32_t ct = 0; ct < CulTypeCount; ct++)
	{
		...
		m_graphics->GetDevice()->CreateRasterizerState(&rsDesc, &m_rStates[ct]);
	}
	...
	for (int32_t i = 0; i < AlphaTypeCount; i++)
	{
		...
		m_graphics->GetDevice()->CreateBlendState(&Desc, &m_bStates[i]);
	}
	...
	for (int32_t f = 0; f < TextureFilterCount; f++)
	{
		for (int32_t w = 0; w < TextureWrapCount; w++)
		{
			...
			m_graphics->GetDevice()->CreateSamplerState(&SamlerDesc, &m_sStates[f][w]);
		}
	}
}
...
//
//----------------------------------------------------------------------------------
RenderState_Imp_DX11::~RenderState_Imp_DX11()
{
	for (int32_t ct = 0; ct < CulTypeCount; ct++)
	{
...
//
//----------------------------------------------------------------------------------
void RenderState_Imp_DX11::Update(bool forced)
{
	...
	if (changeDepth)
	{
		m_graphics->GetContext()->OMSetDepthStencilState(m_dStates[m_next.DepthTest][m_next.DepthWrite], 0);
	}
