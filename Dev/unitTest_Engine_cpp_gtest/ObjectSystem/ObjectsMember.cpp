...
void ObjectSystem_ObjectsMember(bool isOpenGLMode)
{
	int time = 0; 
	...
	engine->Initialize(ace::ToAString("ObjectsMember").c_str(), 640, 480, option);

	AssertObjectsMembers();
...
{
	ObjectSystem_ObjectsMember(true);
}

#if _WIN32
TEST(ObjectSystem, ObjectsMember_DX)
{
	ObjectSystem_ObjectsMember(false);
}
#endif
